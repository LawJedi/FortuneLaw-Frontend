      ...
      <v-container>
        <v-row>
          <v-col cols="8" offset="2">
            <v-row>
              <v-col>
                <p class="text-h2 text-center">Debt Elimination Questionnaire</p>
            ...
            <v-row>
              <span class="text-button section-header">details</span>
            </v-row>
            <v-row>
              <v-col cols="12">
            ...
            <v-row class="flex-column ">
                ...
                >{{errorText}}</v-alert>
                <v-row class="my-2">
                <v-btn @click="submit()" color="secondary" large class="mx-3"><v-icon class="mr-2">save</v-icon>Save</v-btn>
  ...
  methods: {
    submit(){
      ...
      if(qkey == "formid"){
        ...
        let firstName = self.doc.getElementById("first-name").value;
        let lastName = self.doc.getElementById("last-name").value
        req.open("GET", self.ORIGIN+"/client/form/validate?formid="+formid+"&fname="+firstName+"&lname="+lastName);
        req.onreadystatechange = function(){
          if(this.readyState == 4){
            if(this.status == 200){ 
              let agent = JSON.parse(this.response);
              self.save(agent);
            }
          }
        }
    ...
    downloadPDF(){
      ...
      if(!this.pdfGen){
        ...
        window.open("/result/"+this.responseID+"/pdf/download");
      }
    },
    fileSubmit(){
      this.fileSubmitted = false;
    ...
    async save(agentObj){
      ...
      for(let m = 1; m <= mortgages; m++){
        ...
        mortgageFields.forEach(field => {
          ...
          if(el.value === ""){
            el = el.parentElement.parentElement.querySelector("input[type='hidden']") //change the value of vuieitfy text input using js
          }
      ...
      const _data = {
        ...
        "agent_first_name": agentObj.f,
        "agent_last_name": agentObj.l,
        "insurer": "",
        "file_ref": "",
        "agent_email": agentObj.e,
        "agent_id": agentObj.i,
      };
      var xhr = new XMLHttpRequest();
      xhr.open("POST", self.ORIGIN+"/save");
      xhr.setRequestHeader("Content-type", "application/json")
      xhr.setRequestHeader("Authorization", "agent-"+agentObj.k)
